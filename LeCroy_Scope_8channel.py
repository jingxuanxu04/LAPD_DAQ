...
class LeCroy_Scope:
	...
	def displayed_channels(self)  -> ():    # returns a tuple of channel names, e.g. ('C1', 'C4')
		...
		if self.scope.query('C4:TRACE?')[0:2] == 'ON':
			channels += ('C4',)
		if self.scope.query('C5:TRACE?')[0:2] == 'ON':
			channels += ('C5',)
		if self.scope.query('C6:TRACE?')[0:2] == 'ON':
			channels += ('C6',)
		if self.scope.query('C7:TRACE?')[0:2] == 'ON':
			channels += ('C7',)
		if self.scope.query('C8:TRACE?')[0:2] == 'ON':
			channels += ('C8',)			
		return channels
...
scope_ip_addr = '192.168.0.103'   # '128.97.13.149'
