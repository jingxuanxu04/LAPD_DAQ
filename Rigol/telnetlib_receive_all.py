...
import select
import errno
import threading  # Changed from thread

# Check if poll is available (it's part of select module)
...
class Telnet:
    ...
    def __init__(self, host=None, port=0,
        ...
        self.sock = None
        self.rawq = b''  # Changed to bytes
        self.irawq = 0
        self.cookedq = b''  # Changed to bytes
        self.eof = 0
        self.iacseq = b''  # Changed to bytes
        self.sb = 0
        self.sbdataq = b''  # Changed to bytes
    ...
    def msg(self, msg, *args):
        ...
        if self.debuglevel > 0:
            print('Telnet(%s,%s):' % (self.host, self.port), end=' ')  # Changed print syntax
            if args:
                print(msg % args)  # Changed print syntax
            else:
                print(msg)  # Changed print syntax
    ...
    def write(self, buffer):
        ...
        if isinstance(buffer, str):
            buffer = buffer.encode('utf-8')  # Encode strings to bytes
        if IAC in buffer:
    ...
    def read_very_lazy(self):
        ...
        if not buf and self.eof and not self.rawq:
            raise EOFError('telnet connection closed')  # Fixed syntax
        return buf
    ...
    def process_rawq(self):
        """Transfer from raw queue to cooked queue - FIXED for Python 3"""
        ...
        try:
            while self.rawq:
                ...
                if not self.iacseq:
                    if c != IAC:
                        buf[self.sb] = buf[self.sb] + bytes([c])
                        continue
                    else:
                        self.iacseq += bytes([c])
                    ...
                    if c == IAC:
                        ...
                        if self.option_callback:
                            self.option_callback(self.sock, bytes([c]), bytes([0]))
                        else:
                            self.msg('IAC %d not recognized' % c)
                elif len(self.iacseq) == 2:
                    cmd = self.iacseq[1]
                    self.iacseq = b''
                    opt = c
                    if cmd in (DO[0], DONT[0]):  # Compare with byte values
                        self.msg('IAC %s %d', cmd, opt)
                        if self.option_callback:
                            self.option_callback(self.sock, bytes([cmd]), bytes([opt]))
                    elif cmd in (WILL[0], WONT[0]):  # Compare with byte values
                        self.msg('IAC %s %d', cmd, opt)
                        if self.option_callback:
                            self.option_callback(self.sock, bytes([cmd]), bytes([opt]))
        except EOFError:
        ...
        while 1:
            ...
            if self in rfd:
                try:
                    ...
                    print('*** Connection closed by remote host ***')  # Fixed
                    break
                if text:
                    sys.stdout.write(text.decode('utf-8', errors='ignore'))  # Decode bytes
                    sys.stdout.flush()
            if sys.stdin in rfd:
    ...
    def mt_interact(self):
        ...
        threading.Thread(target=self.listener).start()  # Changed from thread
        while 1:
            line = sys.stdin.readline()
            if not line:
                break
            self.write(line.encode('utf-8'))  # Encode to bytes
    ...
    def listener(self):
        ...
        while 1:
            try:
                ...
                return
            if data:
                sys.stdout.write(data.decode('utf-8', errors='ignore'))  # Decode bytes
                sys.stdout.flush()
